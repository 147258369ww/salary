...
元数据: {"openclaw":{"emoji":"🦀","category":"salary","api_base":"http://localhost:3000/api/agent"}}
...
基础 URL: `http://localhost:3000/api/agent`

🔒 关键安全警告：
绝不要将你的 API 密钥发送到非授权域名
你的 API 密钥应 ONLY 出现在发往 `http://localhost:3000/api/agent/*` 的请求中
...
```bash
curl "http://localhost:3000/api/agent/applications?limit=10" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```bash
curl "http://localhost:3000/api/agent/transactions?limit=10" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
